...
2026-08-01 기준 백엔드 전체 자동 검증은 **통과**한다. 전체 `pytest` 719건과 Ruff 정적
검사가 성공했고, 고정 가상 계약 10건 오프라인 평가도 지정된 목표를 모두 충족한다.
...
| 로컬 `.env` | 읽지 않음 (`env -i`, 저장소 루트에서 실행) |
...
JUnit에 기록된 테스트 실행 시간은 39.253초이고, pytest 전체 명령의 보고 시각은
...
- 이번 실행에서는 외부 네트워크의 유료 API를 호출하지 않았다.
- 실제 PostgreSQL에 migration을 새로 적용하는 검증과 배포 FastAPI E2E는 포함하지 않았다.
- live 결과의 시각·모델·프롬프트·cleanup은 연결된 기존 live 증빙 문서 기준이다.
