...
이번 회차의 `pytest`, Ruff, 오프라인 평가는 모두 외부 호출이 없는 `AUTOMATED_OFFLINE`
결과다. 실제 외부 API 결과를 이번 자동 테스트 결과에 섞지 않기 위해, 기존
`LIVE_EXTERNAL` 증빙의 원본 문서를 그대로 참조한다. 이번 세션에서는 live 호출을
다시 실행하지 않았다.
...
실제 실행에는 localhost 목적지, 명시적 `--confirm-live`, 생성 데이터 전용 cleanup 승인이
