...
고정한다. 광고효과 기능은 P2이며 16.1 공통 접근 계층, 기반 migration, 16.2 업로드
endpoint와 16.4 최초 확정·정정 endpoint(revision·flag·문의 문안 원자 저장 포함)는
구현됐다. 16.3 추출과 16.5 집계 조회 endpoint는 아직 미구현이다. 기존 P0 API, 전역
`/dashboard` 응답과 계약 상태 머신은 변경하지 않는다.
