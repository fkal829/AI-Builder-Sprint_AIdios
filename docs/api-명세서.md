...
`refund_condition`은 실제 환불 가능 여부·대상·금액·시기·방식을 정한 문구만 값으로
취급한다. 검증된 원문이 "환불 조건을 계약서에 기재하지 않았다"처럼 명시적 부재만
말하면, 모델이 값을 반환했더라도 모호 표현 판정보다 먼저 `value`, `source_page`,
`source_text`가 `null`이고 `confidence=0`인 `NOT_FOUND`로 정규화해 `MISSING` 후보로
보낸다. "환불하지 않는다", "환불 불가"처럼 비환불이라는 실제 조건을 정한 문구는
명시적 부재로 바꾸지 않고 유효한 환불 조건으로 검증한다.
검증된 원문이 없으면 `MISSING_EVIDENCE`를 유지하고, 같은 원문 근거에 명시적 부재와
실제 환불 조건이 함께 있으면 근거를 보존한 `NEEDS_CHECK`로 처리한다.
