...
> P2 구현 상태: canonical OpenAPI 2/4 `planned`, runtime endpoint 2/4,
> 16.1·16.2·16.3·17.2·17.5 구현
...
신규 4개 operation 중 16.2 리포트 업로드와 16.3 지표 추출은 canonical OpenAPI의
활성 operation과 FastAPI runtime에 등록했고, 16.4~16.5 두 endpoint는 `planned`다. 16.1 공통
...
현재 활성 API 32개의 구현 주 담당은 B 13개, C 19개이며 D가 직접
...
15~19절의 P2 API 4개 중 B 담당 16.2·16.3은 위 현재 구현 개수에 포함했다.
C 담당 16.4·16.5까지 구현하면 활성 API는 34개, 담당은 B 13개·C 21개가 된다.
...
> - 우선순위: P2 발표 로드맵
> - 현재 상태: 프론트엔드 화면 목업과 16.1 공통 기반, 16.2 업로드·16.3 추출 endpoint 구현
> - 기계 계약: canonical OpenAPI 2/4 `planned`, runtime 2/4, 기반 DB migration 구현
...
| 현재 등록 상태 | 16.2·16.3은 canonical OpenAPI·runtime 등록, 16.4~16.5는 `planned` |
...
FastAPI endpoint, 추출 attempt RPC·service·Upstage·Solar 경계와 16.3 endpoint는
runtime에 구현됐다. 16.4~16.5 두 endpoint는 `planned`이며 runtime operation 수는
2/4이다.
