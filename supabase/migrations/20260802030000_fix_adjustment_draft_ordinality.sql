-- PostgreSQL rejects a column definition list attached directly to a record-returning
-- function when WITH ORDINALITY is also present. Keep the public RPC signature and all
-- validation/atomicity rules unchanged, but place the definition list inside ROWS FROM.
